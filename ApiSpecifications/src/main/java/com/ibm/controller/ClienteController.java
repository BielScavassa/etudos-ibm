...
public class ClienteController {
	...
	public ResponseEntity<List<ClienteModel>> clientes(){
		return ResponseEntity.status(HttpStatus.OK).body(clienteService.buscarClientesPorSobrenome(new SearchCriteria("sobrenome", ":", "Sobrenome")));
	}
	
	@GetMapping("/email/{id}")
	public ResponseEntity<String> enderecoCliente(@PathVariable Long id) throws NotFoundException{
		return ResponseEntity.status(HttpStatus.OK).body(clienteService.buscarEmailCliente(id));
	}
